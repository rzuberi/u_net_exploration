...
import torch
import numpy as np
from data_acquiring import get_cellpose_data, get_random_crops, get_data_loaders
...
class CellMaskModel():
    ...
    #TODO: function to save models (not needed)
...
def dice_evaluate(CellMaskModel):
    ...
    for ((x_img,y_img),(x_cp,y_cp)) in zip(CellMaskModel.trainLoader_img,CellMaskModel.trainLoader_cp):
        ...
        dice = CellMaskModel.dice_coeff(mask_tresh,np.squeeze(y_cp.cpu().detach().numpy()))
        dice_coeffs.append(dice)
